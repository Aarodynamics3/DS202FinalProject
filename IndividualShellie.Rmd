...
# What states had the highest amount of cases and deaths over the span of a week? (using cases/states data)
```{r}
mostdeaths7days1 <- covid19DeathsbyState %>%   
  rename(State = `State/Territory`, ) %>% 
  filter(State != 'United States of America', State != 'New York City') %>%
  select(State, `Deaths in Last 7 Days`) %>%
  top_n(5, `Deaths in Last 7 Days`)
str(mostdeaths7days1)
```
```{r}
ggplot(mostdeaths7days1, aes(x= State, weight = `Deaths in Last 7 Days`)) + geom_bar(fill = '#2A3990') + scale_x_discrete(limit = c('California', 'Texas', 'Florida', 'Georgia', 'New Jersey')) + ggtitle('Top 5 States with Most Deaths in Last 7 Days') + xlab('State') + ylab('Deaths in Last 7 Days')
```
