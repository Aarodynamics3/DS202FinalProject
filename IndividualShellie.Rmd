...
# What states had the highest amount of both cases and deaths over the span of a week? (using cases/states data)
```{r}
mostdeaths7days <- covid19DeathsbyState %>%   
  ...
  top_n(7, `Deaths in Last 7 Days`)
str(mostdeaths7days)
```
```{r}
DeathCount <- ggplot(mostdeaths7days, aes(x=State, y=`Deaths in Last 7 Days`)) + geom_point() + ggtitle('Top 7 States with Most Deaths in Last 7 Days') + xlab('State') + ylab('Deaths in Last 7 Days')

DeathCount + geom_abline(intercept = 100, slope = 50, color = "Purple")
```
